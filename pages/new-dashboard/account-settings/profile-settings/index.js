...
const Profile_Settings = (props) => {
  const userData = useSelector((store) => store?.Reducer?.userData?.data)
  ...
  useEffect(() => {
    ...
    if (typeof window !== undefined) {
      if (JSON.parse(localStorage.getItem("USER_DATA_STORAGE"))?.userData?.data?.username !== undefined) {
        userLocaleStorage = JSON.parse(localStorage.getItem("USER_DATA_STORAGE"))?.userData
        store.dispatch({
          type: "NAME",
          action: userLocaleStorage?.data?.username
        })
        store.dispatch({
          type: "userDATA",
          action: userLocaleStorage?.data
        })
      } else {
      ...
      <div className={openSide === true ? "perfect-scrollbar-on nav-open " : "perfect-scrollbar-on"}>
        <div className={openSide === true ? "   " : "sidebar1-mini"}>
          <div className="sidebar-wrapper">
            <SideBar1 user={user} />
            <div style={{ float: "right" }} className={openSide === true ? "main-panel" : "main-panel"}>
              <Navbar1 username={user} openSide={openSide} setOpenSideProps={() => setOpenSide(!openSide)} />
              <Main userLocaleStorage={userdata} />
              <div id="footer">
                <Footer />
                {/* {openSide === false ? (
...
export default withRouter(Profile_Settings)
