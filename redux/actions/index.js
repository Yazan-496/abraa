...
export const _register = async (
  ...
  await API.post(`${process.env.NEXT_PUBLIC_USERS_SERVICE}/public/api/v1/auth/email/signup`, {
    email,
    password,
    mobile_phone,
    signup_country_id,
...
export const getCategoryAttributes = (id, callback, callbackErr) => {
  API.get(`/${process.env.NEXT_PUBLIC_PRODUCTS_SERVICE}/public/api/v1/attributes/category/566`)
    .then(function ({ data }) {
      callback(data)
    })
    .catch(function (res) {})
}
export const getParentCategories = (callback, callbackErr) => {
  API.get(`/${process.env.NEXT_PUBLIC_PRODUCTS_SERVICE}/public/api/v1/categories/roots`)
    .then(function ({ data }) {
      callback(data)
    })
    .catch(function (res) {})
}
export const searchCategories = (search, callback, callbackErr) => {
  const sear = search ? search : "undefined"
  sear !== "undefined" &&
    API.get(`/${process.env.NEXT_PUBLIC_PRODUCTS_SERVICE}/public/api/v1/categories/filter/${sear}`)
      .then(function ({ data }) {
        callback(data)
      })
      .catch(function (res) {})
}
export const getSonsCategories = (parentId, callback, callbackErr) => {
  API.get(`/${process.env.NEXT_PUBLIC_PRODUCTS_SERVICE}/public/api/v1/categories/category_sons/${parentId}`)
    .then(function ({ data }) {
      callback(data)
    })
    .catch(function (res) {})
